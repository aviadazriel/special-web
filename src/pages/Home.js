...
const testimonials = [
...
];const faqData = [
  {
    ...
    answer: `
    ישנם סוגי ריביות שונים במשכנתא, כולל ריבית קבועה, ריבית משתנה וריבית פריים:

    - ריבית קבועה: הריבית נשארת קבועה לאורך כל תקופת ההלוואה, מה שמאפשר תכנון מדויק של ההחזר החודשי ואינו מושפע מהעלאות ריבית עתידיות. זהו מסלול בטוח יותר, אך בדרך כלל הריבית תהיה גבוהה יותר בהשוואה לריבית משתנה.

    - ריבית משתנה: הריבית משתנה בהתאם לשיעור הריבית בשוק, בדרך כלל אחת לכמה חודשים או שנים. זה יכול להוביל לתשלומים חודשיים נמוכים יותר בהתחלה, אך יש סיכון שהריבית תעלה בעתיד, מה שיכול להגדיל את התשלומים החודשיים.

    - ריבית פריים: זוהי ריבית משתנה שמבוססת על ריבית בסיסית, לרוב ריבית בנק ישראל, בתוספת מרווח קבוע. זהו מסלול גמיש שיכול להיות משתלם, אך גם כאן יש סיכון לתנודתיות.
    `,
  },
  {
    question: "איך מתבצע תהליך קבלת המשכנתא?",
    answer: `
    תהליך קבלת המשכנתא כולל מספר שלבים עיקריים:

    1. פגישת ייעוץ: בשלב הראשון, כדאי לקבוע פגישה עם יועץ משכנתאות או בנקאי, שיסייע לכם להבין את האפשרויות הזמינות.

    2. איסוף מסמכים: יש לאסוף את המסמכים הנדרשים, כגון תעודת זהות, תלושי שכר, דוחות מס, ואישור על הכנסות נוספות.

    3. הגשת בקשה לבנק: לאחר שהכנתם את המסמכים, יש להגיש את הבקשה לבנק או לגוף המלווה.

    4. קבלת אישור עקרוני: הבנק יבצע בדיקות ויוציא אישור עקרוני, שמצביע על כך שאתם עומדים בתנאים לקבלת ההלוואה.

    5. חתימה על המשכנתא: לאחר קבלת האישור הסופי, תתבצע החתימה על ההסכם והכספים ישוחררו לרכישת הנכס.
    `,
  },
  {
    question: "מה חשוב לדעת על מחזור משכנתא?",
    answer: `
    מחזור משכנתא הוא תהליך שבו לוקחים הלוואה חדשה כדי להחליף את ההלוואה הקיימת, לרוב במטרה לשפר את התנאים:

    - חיסכון בכסף: מחזור יכול לחסוך כסף במידה והתנאים הכלכליים השתנו (כגון ירידת ריבית) או שהמשכנתא הראשונית לא תואמת לצרכים הנוכחיים שלכם.

    - התאמה לצרכים חדשים: אם המציאות הכלכלית שלכם השתנתה (כמו שינוי בהכנסות או הוצאות), מחזור יכול לאפשר לכם להתאים את המשכנתא למצבכם הנוכחי.

    - ייעוץ מקצועי: חשוב להתייעץ עם מומחה לפני שמבצעים מחזור, שכן ישנם פרטים רבים שצריך לקחת בחשבון כמו עלויות מחזור, ריביות והאם יש קנס על פירעון מוקדם.
    `,
  },
  {
    question: "כיצד יועץ משכנתאות יכול לעזור לי?",
    answer: `
    יועץ משכנתאות מקצועי מציע מגוון שירותים שיכולים להקל עליכם בתהליך רכישת הדירה:

    - הבנת אפשרויות: היועץ יכול לעזור לכם להבין את האפשרויות השונות הקיימות בשוק, ולסייע בבחירה בין סוגי המשכנתאות.

    - תמהיל הלוואה נכון: הוא יכול לסייע לכם לבחור את תמהיל ההלוואה הנכון, כך שתוכלו לקבל את התנאים הטובים ביותר בהתאם למצבכם הכלכלי.

    - שיפור תנאים פיננסיים: יועץ מנוסה יכול לנהל משא ומתן עם הבנקים במקומכם, מה שיכול להוביל לחיסכון משמעותי בהחזר החודשי ובסך ההוצאות הכוללות.

    - ליווי לאורך כל התהליך: יועץ משכנתאות מלווה אתכם בכל שלב בתהליך, מסביר מושגים פיננסיים מורכבים ומסייע בהכנת המסמכים הנדרשים.
    `,
  },
];
const Home = () => {
